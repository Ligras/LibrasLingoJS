...
function exibirQuestao(){
    ...
    document.getElementById("imagem").innerHTML = "<img src="+ matrizImagens[i] + " style=width:200px>";
...
function sortearOpcoes(){
    ...
    do{
        ...
        z = Math.floor(Math.random() * 26);
        w = Math.floor(Math.random() * 26);
    }while( respostaCorreta == opcoes[y] ||  respostaCorreta == opcoes[z] || respostaCorreta == opcoes[w] 
        || opcoes[y] == opcoes[z] || opcoes[y] == opcoes[w] || opcoes[z] == opcoes[w]);

    if(i==0)
        matrizA = [respostaCorreta,opcoes[y],opcoes[z],opcoes[w]];
    else if(i==1)
        matrizA = [opcoes[y],respostaCorreta, opcoes[z],opcoes[w]];
    else if (i==2)
        matrizA = [opcoes[y], opcoes[z],respostaCorreta,opcoes[w]];
    else
        matrizA = [opcoes[y], opcoes[z],opcoes[w],respostaCorreta];
    exibirOpcoes();
}   
